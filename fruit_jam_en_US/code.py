...
menu_grid = GridLayout(x=(display.width // scale - WIDTH) // 2, # tile left right padding
                       y=(display.height // scale - HEIGHT) // 1, # tiles in startup grid below 'ventana 95'
                       width=WIDTH, height=HEIGHT, grid_size=(config["width"], config["height"]),
                       divider_lines=False)
scaled_group.append(menu_grid)

menu_title_txt = Label(font, text="Ventana 25", color=launcher_config.palette_fg)
...
def _create_cell_group(app):
    ...
    icon_tg.x = cell_width // 8 - icon_tg.tile_width // 8
    ...
    icon_tg.y = (cell_height - icon_tg.tile_height - title_txt.height) // 4
